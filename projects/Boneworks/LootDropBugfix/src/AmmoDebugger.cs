...
public class AmmoDebugger {
  ...
  public IEnumerator SpawnAndBreakAmmoCrates() {
    ...
    // is always loaded
    var cratePrefab =
        Object.FindObjectsOfType<ObjectDestructable>()
            .FirstOrDefault(
                obj =>
                    obj.lootTable?.name.StartsWith("AmmoCrateTable_") ?? false
            )
            ?.gameObject;
    ...
    while (head != null && cratePrefab != null && _prefTest.Value) {
      ...
      obj.TakeDamage(
          Vector3.back, 100, false, StressLevelZero.Combat.AttackType.Piercing
      );
      ...
      spawnedLootItem?.GetComponent<Poolee>().Despawn(
          _emptyNullableBool, _emptyNullableColor
      );
      yield return new WaitForSeconds(0.2f / _prefTestSpeed.Value);
    }
  }

  private GameObject FindSpawnedLootItem(
      IEnumerable<string> prefabNames, Vector3 spawnPosition,
    ...
    foreach (var collider in Physics.OverlapSphere(spawnPosition, 0.5f)) {
      ...
      var isCorrectPosition = !checkExactPosition ||
  ...
  public void OnGetLootItem(LootTableData lootTable, SpawnableObject result) {
    if (_prefTest.Value && IsLootGuaranteed(lootTable) && result == null) {
      MelonLogger.Warning("LootTableData.GetLootItem just returned null when " +
                          "loot chances add up to 100%!");
    }
  }
}
