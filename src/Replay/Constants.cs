...
namespace SpeedrunTools.Replay
{
  class Constants
  {
    ...
    public static readonly byte[] MAGIC_HEADER_BYTES = Encoding.ASCII.GetBytes(MAGIC_HEADER);
    public static readonly byte[] FILE_START_BYTES =
      MAGIC_HEADER_BYTES.Concat(System.BitConverter.GetBytes((uint)0)).ToArray();
